...
function playerTurn(playerHand, deck) {
    let handleValue = (0, utils_1.getHandValue)(playerHand);
    while (true) {
        const action = (0, utils_1.getDecision)();
        if (action !== 'hit')
            return handleValue;
        playerHand.push(deck.deal(1)[0]);
        handleValue = (0, utils_1.getHandValue)(playerHand);
        console.log(`Your hand: ${(0, utils_1.getStrHand)(playerHand)} (total: ${handleValue})`);
        if (handleValue > 21) {
            return handleValue;
        }
    }
...
function dealerTurn(dealerHand, deck) {
    let handleValue = (0, utils_1.getHandValue)(dealerHand);
    while (true) {
        console.log(`Dealer's hand: ${(0, utils_1.getStrHand)(dealerHand)} (total: ${handleValue})`);
        if (handleValue >= 17)
            return handleValue;
        dealerHand.push(deck.deal(1)[0]);
        handleValue = (0, utils_1.getHandValue)(dealerHand);
    }
}
let dealerHand = [];
let playerHand = [];
...
while (balance > 0) {
    ...
    console.log(`Your hand: ${(0, utils_1.getStrHand)(playerHand)} (total: ${playerHand})`);
